...
export default class MultiplayerTicTacToe extends LightningElement {
  ...
  get isPlayerXTurn() {
    return (
      this.gameSession?.currentPlayer === "X" &&
  ...
  get isPlayerOTurn() {
    return (
      this.gameSession?.currentPlayer === "O" &&
      this.gameSession?.gameStatus === "In Progress"
    );
  }
  ...
  get showAcceptButton() {
    return (
      this.gameSession?.gameStatus === "Pending" &&
  ...
  async handleCellClick(event) {
    ...
    try {
      ...
      this.showToast(
        "Error",
        error.body?.message || "Failed to make move",
        "error"
      );
    } finally {
