...
export default class TicTacToeLobby extends LightningElement {
  ...
  handleGameEvent(payload) {
    ...
    if (['INVITATION_SENT', 'GAME_STARTED', 'GAME_COMPLETED', 'MOVE_MADE'].includes(eventType)) {
      refreshApex(this.wiredActiveGamesResult);
      
      // Show toast notification for invitations received
      if (eventType === 'INVITATION_SENT' && payload.Player_Id__c !== this.currentUserId) {
  ...
  transformGameData(games) {
    return games.map(game => {
      const opponentName = game.mySymbol === 'X' ? game.playerOName : game.playerXName;
      
      return {
        ...game,
        opponentName,
        statusVariant: this.getStatusVariant(game.gameStatus),
        isPendingInvitation: game.gameStatus === 'Pending' && game.playerOId === this.currentUserId
      };
    });
  }
